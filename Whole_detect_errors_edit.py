...
def find_counts_within_aperture(data, mask, source_map, x_centre, y_centre, aperture_radius, exclude_sources): 
...
def calculate_source_counts_with_double_apertures(source, data, mask, source_map, global_bg,global_bg_err ):
    ...
    inner_ap_counts = find_counts_within_aperture(data,mask,source_map, x_avg_source,y_avg_source, inner_ap_radius,exclude_sources)
    
    exclude_sources = True
    outer_ap_radius = inner_ap_radius+10
    outer_ap_counts = find_counts_within_aperture(data,mask,source_map, x_avg_source,y_avg_source, outer_ap_radius,exclude_sources)
    
    
    source_counts_err_squared = 0
    for count in inner_ap_counts:
        source_counts_err_squared += np.sqrt(count)
        if count in outer_ap_counts:
    ...
    #tot_counts_err = np.sqrt(sum(map(lambda x: x**2, self.pixel_count_errors)) + self.background_contribution_err**2)   
    if sum(inner_ap_counts) - local_bg_emission * len(inner_ap_counts) > 0:
        source_emission = sum(inner_ap_counts) - local_bg_emission * len(inner_ap_counts)
        source_emission_err = np.sqrt(source_counts_err_squared + len(inner_ap_counts)*(local_bg_emission_err**2))
    else:
        source_emission = sum(inner_ap_counts) - global_bg * len(inner_ap_counts)
        source_emission_err = np.sqrt(source_counts_err_squared + len(inner_ap_counts)*(global_bg_err**2))        
    

   # source_emission_err = local_bg_emission_err * np.sqrt(len(inner_ap_counts))
...
for ob in iteration_list:
    ...
    source_brightness, source_brightness_err = calculate_source_counts_with_double_apertures(ob,data,mask, source_map, background, global_bg_err)
...
print("Lower pixel limit",lower_pix_limit)
num_pix_unmasked =0
